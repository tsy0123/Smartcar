...
unsigned char Bin_Pixle[94][60];

extern bool show_Road;
...
#define BUZZ          IfxGtm_ATOM2_4_TOUT30_P33_8_OUT
/*!
 * @brief    摄像头测试例程
 *
 * @param
    ...
    while (1)
    {
        if (Camera_Flag == 2)
        {
            ...
            if(flag_isRight_ring == 1 || flag_isLeft_ring == 1)
                ...
                LED_Ctrl(LED0,OFF);
            if(flag_crossLeft_find == 1 || flag_crossRight_find == 1)
                LED_Ctrl(LED1,ON);
            else
                LED_Ctrl(LED1,OFF);
...
void CAMERA_Reprot (void)
{
    ...
    for (i = 0; i < 60; i++)
    {
        for (j = 0; j < 12; j++)
        {
            if(Bin_Pixle[j*8][i])
                sendBuffer = 1;
            if(Bin_Pixle[j*8+1][i])
                sendBuffer |= 0x02;
            if(Bin_Pixle[j*8+2][i])
                sendBuffer |= 0x04;
            if(Bin_Pixle[j*8+3][i])
                sendBuffer |= 0x08;
            if(Bin_Pixle[j*8+4][i])
                sendBuffer |= 0x10;
            if(Bin_Pixle[j*8+5][i])
                sendBuffer |= 0x20;
            if(j != 11)
            {
                if(Bin_Pixle[j*8+6][i])
                    sendBuffer |= 0x40;
                if(Bin_Pixle[j*8+7][i])
                    sendBuffer |= 0x80;
            }
    ...
    for (i = 0; i < 60; i++)
    {
        if(imageLine.Exist_Left[i])
            sendBuffer = 0x80 + imageLine.Point_Left[i];
        else
            sendBuffer = 0;
        UART_PutChar(UART0, sendBuffer);
    }
    for (i = 0; i < 60; i++)
    {
        if(imageLine.Exist_Right[i])
            sendBuffer = 0x80 +imageLine.Point_Right[i];
        else
            sendBuffer = 0;
        UART_PutChar(UART0, sendBuffer);
    }
    for (i = 0; i < 60; i++)
    {
        if(imageLine.Exist_Center[i] && !imageLine.Lost_Center)
            sendBuffer = 0x80 +imageLine.Point_Center[i];
        else
            sendBuffer = 0;
        UART_PutChar(UART0, sendBuffer);
    }
...
void Get_Bin_Image (unsigned char mode)
{
    ...
    {
        Threshold = 80;                          //手动调节阈值
        lq_sobel(Image_Use, Bin_Pixle, (unsigned char) Threshold);
    ...
    {
        lq_sobelAutoThreshold(Image_Use, Bin_Pixle);  //动态调节阈值
        return;
    }
/*
#ifdef USEOLED
...
*/
    ...
    for (i = 0; i < 60; i++)
    {
        for (j = 0; j < 94; j++)
        {
            if (Image_Use[j][i] > Threshold) //数值越大，显示的内容越多，较浅的图像也能显示出来
                Bin_Pixle[j][i] = 1;
            else
                Bin_Pixle[j][i] = 0;
        }
...
*************************************************************************/
short GetOSTU (unsigned char tmImage[LCDW][LCDH])
{
    signed short i, j;
 ...
 * @date     2020/5/15
 */
void lq_sobel (unsigned char imageIn[LCDW][LCDH], unsigned char imageOut[LCDW][LCDH], unsigned char Threshold)
{
    /** 卷积核大小 */
    short KERNEL_SIZE = 3;
 ...
 * @date     2020/5/15
 */
void lq_sobelAutoThreshold (unsigned char imageIn[LCDW][LCDH], unsigned char imageOut[LCDW][LCDH])
{
    /** 卷积核大小 */
    short KERNEL_SIZE = 3;
    short xStart = KERNEL_SIZE / 2;
