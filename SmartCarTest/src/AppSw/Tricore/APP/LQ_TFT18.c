...
void LCD_ShowPictureBin(u16 x,u16 y,u16 length,u16 width,const u8 pic[])
{
    ...
    for(i=0;i<60;i++)
    {
        for(j=0;j<94;j++)
        {
            ...
            {
                if(j == imageLine.Point_Left[i])
                    TFTSPI_Write_Word (0x07E0);
            }
            else if(imageLine.Exist_Right[i] == 1&&(j == imageLine.Point_Right[i]))
            {
                if(j == imageLine.Point_Right[i])
                    TFTSPI_Write_Word (0x07FF);
            }
            else if(imageLine.Exist_Center[i] == 1&&(j == imageLine.Point_Center[i]))
            {
                if(i == AIM_LINE)
                    TFTSPI_Write_Word (0xFFE0);
                else
                    TFTSPI_Write_Word (0xf000);
            }
            else if(j == 47)
            {
                TFTSPI_Write_Word (0x780F);
            }
            else if(pic[i+j*60])
            {
                /* 显示 */
                TFTSPI_Write_Word (0x7BEF);
            }
            else
            {
                /* 显示 */
...
void LCD_ShowPicture(u16 x,u16 y,u16 length,u16 width,const u8 pic[])
{
    ...
    for(i=0;i<width;i++)
    {
        for(j=0;j<length;j++)
        {
            ...
            {
                color = (pic[i+j*60] >> 3) << 11;
                color |= (pic[i+j*60] >> 2) << 5;
                color |= pic[i+j*60] >> 3;
