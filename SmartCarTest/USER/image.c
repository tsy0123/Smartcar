...
void forkRoad_find(void)
{
    ...
    if(isForkRoadTurnLeft)
    {
        ...
        {
            for (i = MIN(turnPoint_Left,turnPoint_Right)-2; i > FORK_DETECT_TURN_END_ROW; i--)
            {
                if(White_Black_White_detect(i,2))
                {
                    for(uint8_t j = i + 2; j < MIN(turnPoint_Left,turnPoint_Right) - 1; j++)
                    {
        ...
        {
            /*for(i = turnPoint_Left; i > FORK_DETECT_TURN_END_ROW; i--)
            {
            ...
            for (i = turnPoint_Left-2; i > FORK_DETECT_TURN_END_ROW; i--)
            {
                if(White_Black_White_detect(i,2))
                {
                    for(uint8_t j = i + 2; j < turnPoint_Left - 1; j++)
                    {
                        if(imageLine.White_Num[j +1] > imageLine.White_Num[j])
                        {
                            flag_forkRoad_prefind = 0;
            ...
            if(flag_forkRoad_prefind)
            {
                ...
                for(uint8_t j = left_point; j > left_point - 4; j--)
                {
                    if(!White_Black_White_detect(j,2) || imageLine.White_Num[j] < imageLine.White_Num[j - 1] + 4)
                    {
                        flag_forkRoad_prefind = 0;
                        return;
                    }
        ...
        else if(turnPoint_Right > 0)
        {
            ...
            for(j = i; j > i - 4; j++)
            {
                if(!White_Black_White_detect(i,2) || !(imageLine.White_Num[j]>imageLine.White_Num[j-1]))
                {
                    flag_forkRoad_prefind = 0;
                    return;
                }
        ...
        else if(imageLine.Lost_Left || imageLine.Lost_Right)
        {
            ...
            for(i = FORK_DETECT_LOST_TURN_ROW; i > 0; i--)
            {
                if(White_Black_White_detect(i,2))
                {
                    if(White_Black_White_detect(i - 1,2) && White_Black_White_detect(i - 2,2) && White_Black_White_detect(i - 3,2)\
                            &&  imageLine.White_Num[i]>imageLine.White_Num[i-1]+4 && imageLine.White_Num[i-1]>imageLine.White_Num[i-2]+4\
                            && imageLine.White_Num[i-2]>imageLine.White_Num[i-3]+4)
                    {
                        fork_Point_row = i;
                        flag_forkRoad_prefind = 1;
    ...
    else
    {
        ...
        else if(imageLine.Lost_Right || imageLine.Lost_Left)
        {
            uint8_t isWBW = 0;
...
void ring_LostCenter_mend(void)
{
    ...
    if (flag_isLeft_ring && !find_ring_Left)
    {
        ...
        if((imageLine.Lost_Left &&  imageLine.Lost_Right) || flag_LostCenter_Left)
        {
            imageLine.Lost_Right = true;
            imageLine.Exist_Center[AIM_LINE_SET] = true;
    ...
    else if (flag_isRight_ring && !find_ring_Right)
    {
        ...
        if((imageLine.Lost_Left &&  imageLine.Lost_Right)|| flag_LostCenter_Right)
        {
            imageLine.Lost_Right = true;
            imageLine.Exist_Center[AIM_LINE_SET] = true;
...
/*****************************************************************
*Function: cross_JudgeLeft(*)
*Description: 斜入十字防串道(偏左) 检测与滤点
*parameter: *
*Return: *
*****************************************************************/
void cross_JudgeLeft(void)
{
    short i, j;

    /************************求左边界点最右端值***********************/
    uint8_t leftLineMAX = 0;
    for (i = 0; i < MT9V03X_H; i++)
    {
    ...
    //若最右未超过1/3则不检查(小S直接return)
    if (leftLineMAX < MT9V03X_W / 3)
        return;

    //定义左拐点1 2
    uint8_t turnPoint_L1 = EFFECTIVE_ROW;
    uint8_t turnPoint_L2 = EFFECTIVE_ROW;

    /************************求左线拐点1***********************/
    for (i = MT9V03X_H - 2; i >= EFFECTIVE_ROW; i--)//从下往上
    {
        if (imageLine.Exist_Left[i])//该行存在左边界点
        {
            for (j = i + 1; j < MT9V03X_H; j++)//从找到的左边界点向下找临近左边界点
            {
                if (imageLine.Exist_Left[j])
                {
        ...
        //只要找到拐点1就直接break
        if (turnPoint_L1 != EFFECTIVE_ROW)
            break;
    }

    /************************求左线拐点2***********************/
    for (i = turnPoint_L1 - 1; i >= EFFECTIVE_ROW; i--)//从拐点1的上面一行继续搜索
    {
        if (imageLine.Exist_Left[i])//该行存在左边界点
        {
            for (j = i + 1; j < MT9V03X_H; j++)//从找到的左边界点向下找临近左边界点
        ...
        //只要找到拐点2就直接break
        if (turnPoint_L2 != EFFECTIVE_ROW)
            break;
    }

    /************************最小二乘法拟合两段直线***********************/
    float K_Left1, B_Left1, K_Left2, B_Left2;//拟合线段1，2的K,B
    int Sum_XY = 0, Sum_X = 0, Sum_Y = 0, Sum_XX = 0;
    float Aver_X, Aver_Y;
    short ValidCnt = 0;

    //拟合线段1
    for (i = turnPoint_L1; i < MT9V03X_H; i++)
    {
        if (imageLine.Exist_Left[i])
        {
            Sum_X += i;
            Sum_Y += imageLine.Point_Left[i];
            Sum_XX += i * i;
            Sum_XY += i * imageLine.Point_Left[i];

            ValidCnt++;
        }
    }
    if (ValidCnt >= CROSS_EDGE_LEN)//拟合十字路口两段直线时所需的最少点数
    {
    ...
    for (i = turnPoint_L2; i < turnPoint_L1; i++)
    {
        if (imageLine.Exist_Left[i])
        {
            Sum_X += i;
    ...
    float Fit_Err1 = 0, Fit_Err2 = 0;//拟合误差
    uint8_t cnt1 = 0, cnt2 = 0;
    float Fit_Val;

    for (i = turnPoint_L2; i < MT9V03X_H; i++)
    {
        if (imageLine.Exist_Left[i])
        {
            if (i < turnPoint_L1)
            {   //第二段
                ...
                Fit_Err2 += ((imageLine.Point_Left[i] - Fit_Val) * (imageLine.Point_Left[i] - Fit_Val));
                cnt2++;
            }
            else
            {   //第一段
                Fit_Val = K_Left1 * i + B_Left1;
                Fit_Err1 += ((imageLine.Point_Left[i] - Fit_Val) * (imageLine.Point_Left[i] - Fit_Val));
                cnt1++;
            }
        }
    }
    ...
    if (
        ...
        ((K_Left1 < -2 && K_Left2 > 1) || (K_Left1 < 0 && K_Left2 > 2.5f))
        )
    {
        for (i = EFFECTIVE_ROW; i < turnPoint_L1; i++)
        {
            imageLine.Exist_Left[i] = false;
        }
...
/*****************************************************************
*Function: cross_JudgeLeft(*)
*Description: 斜入十字防串道(偏右) 检测与滤点
*parameter: *
*Return: *
*****************************************************************/
void cross_JudgeRight(void)
{
    short i, j;

    /************************求右边界点最右端值***********************/
    uint8_t rightLineMAX = 0;
    for (i = EFFECTIVE_ROW; i < MT9V03X_H; i++)
    {
    ...
    //若最左点未小于2/3则不检查(小S直接return)
    if (rightLineMAX > 2 * MT9V03X_W / 3)
        return;

    //定义右拐点1 2
    uint8_t turnPoint_R1 = EFFECTIVE_ROW;
    uint8_t turnPoint_R2 = EFFECTIVE_ROW;

    /************************求右线拐点1***********************/
    for (i = MT9V03X_H - 2; i >= EFFECTIVE_ROW; i--)//从下往上
    {
        if (imageLine.Exist_Right[i])//该行存在右边界点
        {
            for (j = i + 1; j < MT9V03X_H; j++)//从找到的右边界点向下找临近右边界点
            {
                if (imageLine.Exist_Right[j])
                {
        ...
        //只要找到拐点1就直接break
        if (turnPoint_R1 != EFFECTIVE_ROW)
            break;
    }

    /************************求右线拐点2***********************/
    for (i = turnPoint_R1 - 1; i >= EFFECTIVE_ROW; i--)//从拐点1的上面一行继续搜索
    {
        if (imageLine.Exist_Right[i])//该行存在左边界点
        {
            for (j = i + 1; j < MT9V03X_H; j++)//从找到的左边界点向下找临近左边界点
            {
                if (imageLine.Exist_Right[i])
                {
                    //如果趋势逆转，即下方右边界点在上方右边界点的右边
                    if (imageLine.Point_Right[j] > imageLine.Point_Right[i])
                    {
                        turnPoint_R2 = i;
                        break;
    ...
    //拟合线段1
    for (i = turnPoint_R1; i < MT9V03X_H; i++)
    {
        if (imageLine.Exist_Right[i])
        {
            Sum_X += i;
            Sum_Y += imageLine.Point_Right[i];
            Sum_XX += i * i;
            Sum_XY += i * imageLine.Point_Right[i];
    ...
    for (i = turnPoint_R2; i < turnPoint_R1; i++)
    {
        if (imageLine.Exist_Right[i])
        {
            Sum_X += i;
    ...
    for (i = turnPoint_R2; i < MT9V03X_H; i++)
    {
        if (imageLine.Exist_Right[i])
        {
            if (i < turnPoint_R1)
            {   //第二段
                ...
                Fit_Err2 += ((imageLine.Point_Right[i] - Fit_Val) * (imageLine.Point_Right[i] - Fit_Val));
                cnt2++;
            }
            else
            {   //第一段
                Fit_Val = K_Right1 * i + B_Right1;
                Fit_Err1 += ((imageLine.Point_Right[i] - Fit_Val) * (imageLine.Point_Right[i] - Fit_Val));
                cnt1++;
            }
        }
    }
    ...
    if (
        ((Fit_Err1 <= 4 && Fit_Err2 <= 1) || (Fit_Err1 <= 1 && Fit_Err2 <= 4)) &&
        ((K_Right1 > 2 && K_Right2 < -1) || (K_Right1 > 0 && K_Right2 < -2.5f))
        )
    {
        for (uint8_t i = EFFECTIVE_ROW; i < turnPoint_R1; i++)
        {
            imageLine.Exist_Right[i] = false;
        }
        for (uint8_t i = EFFECTIVE_ROW; i < MT9V03X_H; i++)
        {
            imageLine.Exist_Left[i] = false;
        }
        imageLine.Lost_Left = true;
...
void crossFilter_tsy(void)
{
    ...
    for(i = MT9V03X_H-3; i > MT9V03X_H_6; i--)
    {
        ...
        if(flag_left == 1 && isWhite(10, i) && isWhite(10, i - 1) && isWhite(10, i - 2) && turn_row_left-i < 6)
        {
            turn_row_left = i;
            flag_left++;
            //printf("b%d\r\n",turn_row);
        }
        if(flag_right == 1 && isWhite(MT9V03X_W - 10, i) && isWhite(MT9V03X_W - 10, i - 1) &&\
                isWhite(MT9V03X_W - 1, i - 2) && turn_row_right-i < 6)
        {
            turn_row_right = i;
            flag_right++;
        }
        ...
        if(flag_fullcross_right&&flag_fullcross_left)
        {
            /*flag_crossLeft_find = 0;
            flag_crossRight_find = 0;*/
        }
    ...
    if(!flag_fullcross_right && !flag_fullcross_left)
    {
        if(imageLine.White_Num[58] > IS_WHITE_ROW_NUM)
        {
            for(i = 58; i > 20; i--)
            {
                if(imageLine.White_Num[i] > imageLine.White_Num[i-2] + 20)
                {
                    flag_fullcross_left = 1;
                    flag_fullcross_right = 1;
            ...
            if(crossLeft_flag==2)
            {
                for(uint8_t j = 40; j < 60; j++)
                {
                    if(imageLine.Exist_Left[j] && imageLine.Exist_Left[j-1] && imageLine.Exist_Left[j-2])
                        if(imageLine.Point_Left[j]>imageLine.Point_Left[j-1] && imageLine.Point_Left[j-1]>imageLine.Point_Left[j-2])
                        {
                            flag_crossLeft_find++;
                            crossLeft_flag=0;
                        }
            ...
            if(crossRight_flag==2)
            {
                for(uint8_t j = 40; j < 60; j++)
                {
                    if(imageLine.Exist_Right[j] && imageLine.Exist_Right[j-1] && imageLine.Exist_Right[j-2])
                        if(imageLine.Point_Right[j]<imageLine.Point_Right[j-1] && imageLine.Point_Right[j-1]<imageLine.Point_Right[j-2])
                        {
                            crossRight_flag=0;
...
void crossDetect_Left_tsy(void)
{
    ...
    if(!imageLine.Exist_Left[MT9V03X_H-3] && imageLine.Exist_Right[MT9V03X_H-3])
...
{
    uint8_t i,j,isWBW_flag = 0,count = 0,line = 0;
    if(!imageLine.Exist_Right[MT9V03X_H-3] && imageLine.Exist_Left[MT9V03X_H-3])
...
void crossout_mend(void)
{
    ...
    if (flag_crossLeft_find)
    {
        ...
        if((flag_LostCenter_Left) || find_cross_Lost)
        {
        ...
        if(find_cross_Lost && !imageLine.Lost_Left && !imageLine.Lost_Right)
        {
    ...
    else if (flag_crossRight_find)
    {
        ...
        if(count>CROSS_LOST_ROW_COUNT)
        //if(imageLine.White_Num[59]>IS_WHITE_ROW_NUM && black_count > 0)
        {
            flag_LostCenter_Right = 1;
        }
        if((flag_LostCenter_Right) || find_cross_Lost)
        {
            find_cross_Lost = 1;
...
void doMend(void)
{
        ...
        if(!find_ring_flag_Left && !find_ring_flag_Right && !flag_isRight_ring && !flag_isLeft_ring)
        ...
        if (flag_forkRoad_find == 1 && (!isLeftLineStraight || !isRightLineStraight) && !flag_isRight_ring && !flag_isLeft_ring\
                && !flag_crossLeft_find && !flag_crossRight_find)
...
void StraightLineJudge(void)
{
    ...
    //拟合左线---------------------
    for (i = EFFECTIVE_ROW+10; i < MT9V03X_H; i++)
    {
        if (imageLine.Exist_Left[i])
        {
    ...
    if (imageLine.Lost_Left && !imageLine.Lost_Right)
    {
        ...
        //左线丢了 再找一次左线----------
        for (i = EFFECTIVE_ROW+10; i < MT9V03X_H; i++)//从上往下
        {
